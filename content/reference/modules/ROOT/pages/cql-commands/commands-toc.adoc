...
xref:cql-commands/alter-user.adoc[ALTER USER (Deprecated)] ::
Deprecated. Alter existing user options.

xref:cql-commands/batch.adoc[BATCH] ::
Applies multiple data modification language (DML) statements with atomicity and/or in isolation.

xref:cql-commands/create-aggregate.adoc[CREATE AGGREGATE] ::
Defines a user-defined aggregate.
xref:cql-commands/create-custom-index.adoc[CREATE CUSTOM INDEX] ::
Creates a storage-attached index.

xref:cql-commands/create-function.adoc[CREATE FUNCTION] ::
Creates custom function to execute user provided code.

xref:cql-commands/create-index.adoc[CREATE INDEX] ::
Defines a new index for a single column of a table.

xref:cql-commands/create-keyspace.adoc[CREATE KEYSPACE] ::

xref:cql-commands/create-materialized-view.adoc[CREATE MATERIALIZED VIEW] ::
Optimizes read requests and eliminates the need for multiple write requests by duplicating data from a base table.

xref:cql-commands/create-role.adoc[CREATE ROLE] ::
Creates a cluster wide database object used for access control.

xref:cql-commands/create-table.adoc[CREATE TABLE] ::
Creates a new table.

xref:cql-commands/create-type.adoc[CREATE TYPE] ::
Creates a custom data type in the keyspace that contains one or more fields of related information.

xref:cql-commands/create-user.adoc[CREATE USER (Deprecated)] ::
Deprecated. Creates a new user.

xref:cql-commands/delete.adoc[DELETE] ::
Removes data from one or more columns or removes the entire row

xref:cql-commands/drop-aggregate.adoc[DROP AGGREGATE] ::
Deletes a user-defined aggregate from a keyspace.

xref:cql-commands/drop-function.adoc[DROP FUNCTION] ::
Deletes a user-defined function (UDF) from a keyspace.

xref:cql-commands/drop-index.adoc[DROP INDEX] ::
Removes an index from a table.

xref:cql-commands/drop-keyspace.adoc[DROP KEYSPACE] ::
Removes the keyspace.

xref:cql-commands/drop-materialized-view.adoc[DROP MATERIALIZED VIEW] ::
Removes the named materialized view.

xref:cql-commands/drop-role.adoc[DROP ROLE] ::
Removes a role.

xref:cql-commands/drop-table.adoc[DROP TABLE] ::
Removes the table.

xref:cql-commands/drop-type.adoc[DROP TYPE] ::
Drop a user-defined type.

xref:cql-commands/drop-user.adoc[DROP USER (Deprecated)] ::
Removes a user.

xref:cql-commands/grant.adoc[GRANT] ::
Allow access to database resources.

xref:cql-commands/insert.adoc[INSERT] ::
Inserts an entire row or upserts data into existing rows.

xref:cql-commands/list-permissions.adoc[LIST PERMISSIONS] ::
Lists permissions on resources.

xref:cql-commands/list-roles.adoc[LIST ROLES] ::
