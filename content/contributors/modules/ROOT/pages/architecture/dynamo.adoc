...
Simple single-token consistent hashing creates a balancing problem in
small clusters: each node owns one large token range, so adding a new
node forces a few large ranges to be split unevenly.
Cassandra seeks to avoid that imbalance because uneven token ranges lead
directly to uneven request load and uneven disk usage.
In the eight-node example above, there is no clean way to add a ninth
single token and keep the ring balanced.

The Dynamo paper's solution is to use "virtual nodes".
Virtual nodes assign multiple tokens to each physical node, so one new
machine can take many small slices from many neighbors instead of one
large slice from one neighbor.
That makes incremental scale-out much smoother and keeps ownership more
evenly distributed.
...
supported. You also can't use `LWT`, logged batches, or counters with transient replication.
