...
and doesn't fully overlap with the xref:operators::upgrade/onboarding-to-accord.adoc[user guide]. You should start with the
xref:operators::upgrade/onboarding-to-accord.adoc[user guide] to get any context that may be missing here.
...
[Paxos only]
    |
    | phase 1 begins: Accord handles non-SERIAL writes, Paxos still handles SERIAL reads/writes
    v
[Phase 1: dual system, Paxos reads]
    |
    | full/data repair completes and Paxos key barriers make reads safe
    v
[Phase 2: Accord reads, key barrier active]
    |
    | remaining Paxos-owned reads/writes drained, Accord takes full ownership
    v
[Accord only]
----
