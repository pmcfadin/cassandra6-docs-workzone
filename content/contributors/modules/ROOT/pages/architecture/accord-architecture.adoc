...
If you are not already comfortable with Cassandra's non-transactional model, read these first:

* xref:architecture/dynamo.adoc[Dynamo] -- partitioning, replication, and last-write-wins background
* xref:architecture/accord.adoc[Accord] -- what problem Accord solves and where it fits in Cassandra

If you know Raft/Paxos basics but not Accord yet, the minimum concepts you need here are: coordinator, replica quorum, proposed execution timestamp, and durable recovery of in-flight transactions.
...
At a high level, the coordinator is solving one problem: get a quorum of replicas to agree on what transaction is running, when it will execute, and what dependencies must be respected before commit.
...
`CommandStore` exists to solve a practical replica-side problem: many Accord messages can target the same shard at once, but replica metadata must still advance in a deterministic order.
The store is the shard-local execution lane for Accord metadata, dependencies, and durability bookkeeping.
...
Think of `SafeCommandStore` as the transactionally mutable view of one shard's command state for a single operation.
It lets the replica load what it needs, apply the current message, and publish the result atomically when the operation completes.
