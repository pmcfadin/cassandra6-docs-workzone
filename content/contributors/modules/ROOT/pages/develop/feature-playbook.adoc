...
* **Bug fixes**: target the earliest affected release branch (e.g., `cassandra-4.1` if the bug exists there).
  Fixes merge forward through newer branches to `trunk`; committers handle those forward-merges after the patch lands, not contributors.
...
Distributed tests (dtests) run against real multi-node clusters in the separate `apache/cassandra-dtest` repository.
