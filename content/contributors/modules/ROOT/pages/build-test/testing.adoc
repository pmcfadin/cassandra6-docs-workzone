...
They live in a separate repository: https://github.com/apache/cassandra-dtest[apache/cassandra-dtest].
...
Use the Docker path when you want CI parity or you are validating a failure that may be environment-sensitive.
Local Ant runs are faster for iteration.
Docker adds startup overhead, so expect a small test target to take a little longer than the equivalent Ant run.
...
Debug-mode runs are slower than normal test runs because they keep the JVM open for a debugger connection.
