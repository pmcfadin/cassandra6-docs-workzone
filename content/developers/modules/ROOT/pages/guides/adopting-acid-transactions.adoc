...
ACID transactions are a correctness feature, not a general replacement for normal Cassandra writes.
They let you move multi-step coordination logic out of the application and into Cassandra when the workflow truly needs atomicity and serializable isolation.
...
If your application already models a workflow as independent writes plus compensating actions, ACID transactions can often simplify that logic.
If the workflow is naturally a single-partition write, keep it that way.
...
[TIP]
====
A good transaction usually reads a few rows, checks one or two business conditions, and writes a few rows.
If the block starts to look like a batch job or a mini stored procedure, the design likely needs to be simplified.
====
