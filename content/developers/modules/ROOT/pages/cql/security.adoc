...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/role_name.bnf[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/create_role_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_role.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_role_ifnotexists.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/alter_role_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/alter_role.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/drop_role_statement.bnf[]
----
...
xref:operators::secure/security.adoc#authorization[authorization].
...
subject to the xref:operators::secure/security.adoc#auth-caching[caching options] configured in xref:reference::configuration/cassandra_yaml_file.adoc[cassandra-yaml] file.
Should a dropped role be subsequently recreated and have new xref:cql/security.adoc#grant-permission-statement[permissions] or
xref:cql/security.adoc#grant-role-statement[roles] granted to it, any client sessions still
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/grant_role_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/grant_role.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/role_error.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/revoke_role_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/revoke_role.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/list_roles_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list_roles.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list_roles_of.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list_roles_nonrecursive.cql[]
----

[[list-superusers-statement]]
== LIST SUPERUSERS

_Added in Cassandra 6.0 (CASSANDRA-19417)._

All roles with effective superuser privilege — including those that have acquired superuser status
transitively through role inheritance — can be listed using the `LIST SUPERUSERS` statement:

[source, bnf]
----
include::Cassandra:cassandra:example$BNF/list_superusers_statement.bnf[]
----
...
The statement returns a single-column result set:
...
The `role` column contains the name of each role that has superuser access.
This includes roles where `SUPERUSER = true` is set directly, and also roles that have acquired
superuser status through a chain of role grants.

This command requires `DESCRIBE` permission on all roles of the database
(`RoleResource.root()`). A role without this permission receives an `UnauthorizedException`.

`LIST SUPERUSERS` differs from `LIST ROLES` in that it returns only roles with effective superuser
access and resolves role hierarchy automatically, whereas `LIST ROLES` returns all roles with a
`superuser` column that the caller must interpret.

Audit logging records `LIST SUPERUSERS` operations using the `LIST_SUPERUSERS` entry type
in the `DCL` audit category
(source: `src/java/org/apache/cassandra/cql3/statements/ListSuperUsersStatement.java`,
`src/java/org/apache/cassandra/auth/Roles.java` `getAllRoles(Predicate)`; CASSANDRA-19417).
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/create_user_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_user.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_user_role.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/alter_user_statement.bnf[]
----
If the role does not exist, the statement will return an error, unless `IF EXISTS` is used in which case the operation is a no-op.
For example:
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/alter_user.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/drop_user_statement.bnf[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/list_users_statement.bnf[]
----

Note that this statement is equivalent to xref:cql/security.adoc#list-roles-statement[LIST ROLES], but only roles with the `LOGIN` privilege are included in the output.
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/grant_permission_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/grant_perm.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/grant_modify.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/grant_drop.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/grant_execute.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/grant_describe.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/revoke_permission_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/revoke_perm.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/no_revoke.cql[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/list_permissions_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list_perm.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list_all_perm.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list_select_perm.cql[]
----
