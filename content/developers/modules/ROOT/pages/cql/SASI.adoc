...
[IMPORTANT]
====
SASI is a legacy index implementation and is no longer recommended for new development.
Use xref:cql/indexing/sai/sai-concepts.adoc[Storage-Attached Indexing (SAI)] for new indexing work, and see xref:guides/sai-usage-patterns.adoc#migrating-from-sasi-to-sai[Migrating from SASI to SAI] when replacing existing SASI-backed query paths.
====
