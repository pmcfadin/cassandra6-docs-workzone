...
Compared with legacy secondary indexes (2i), SAI is more flexible for multi-column filtering, generally uses less disk, and is better aligned with wide-table Cassandra workloads.
2i is still useful for simple single-column lookups, but SAI is the stronger fit when you need multiple predicates or range filtering on supported types.
...
include::Cassandra:cassandra:partial$sai/support-databases.adoc[]
...
See xref:Cassandra:cassandra:getting-started/sai-quickstart.adoc[SAI quickstart].
...
Refer to xref:Cassandra:cassandra:developing/cql/indexing/sai/operations/configuring.adoc[Configure SAI indexes].
...
Start in xref:Cassandra:cassandra:getting-started/sai-quickstart.adoc[SAI quickstart].

* Rebuilt and backed up via nodetool.
See xref:reference::nodetool/nodetool.adoc[nodetool].

* Monitored via a combination of nodetool, CQL virtual tables, system metrics, and JMX.
See xref:cql/indexing/sai/operations/monitoring.adoc[Monitor SAI indexes].
...
// include::Cassandra:cassandra:partial$sai/config-limits.adoc[]
...
Starting in Cassandra 6.0, SAI additionally supports element-level (`VALUES`, `KEYS`, `ENTRIES`) indexing on frozen collections.
See <<saiFrozenCollectionFaq,frozen collection FAQ>> for details.

--
include::Cassandra:cassandra:partial$sai/collections-note.adoc[]
--
...
--
include::Cassandra:cassandra:partial$sai/queryable-paragraph.adoc[]
--

See xref:cql/indexing/sai/operations/monitoring.adoc#saiMonitorVirtualTables[Virtual tables] and xref:reference::sai-virtual-table-indexes.adoc[Virtual tables for SAI indexes and SSTables].
...
See xref:cql/indexing/sai/sai-read-write-paths.adoc[SAI write path and read path].
...
[NOTE]
====
* `INET` support for IPv4 and IPv6 was new starting with Cassandra 5.0.
* The `DECIMAL` and `VARINT` support was new starting with Cassandra 5.0.
...
* xref:cql/indexing/sai/collections.adoc[Using collections with SAI]
// * xref:Cassandra:cassandra:reference/cql-commands/create-custom-index.adoc[CREATE CUSTOM INDEX]

include::Cassandra:cassandra:partial$sai/collections-note.adoc[]

[[saiFrozenCollectionFaq]]
== Does SAI support element-level indexing on frozen collections?

Yes, starting in Cassandra 6.0 (CASSANDRA-18492).
SAI now accepts `VALUES`, `KEYS`, and `ENTRIES` index targets on frozen collections, in addition to `FULL`.
This enables `CONTAINS`, `CONTAINS KEY`, and map entry equality queries on individual elements within frozen sets, lists, and maps.

Before Cassandra 6.0, all index types (SAI and non-SAI) required `FULL` indexing for frozen collections, which only supports exact equality on the complete serialized collection value.
In Cassandra 6.0, this restriction is relaxed for SAI only -- non-SAI (legacy 2i) indexes still require `FULL` for frozen collections.

See xref:cql/indexing/sai/collections.adoc#saiUsingFrozenCollections[Frozen collection indexing] for examples and a full list of supported index targets.

[NOTE]
====
`ENTRIES` indexing on a frozen map column that is a clustering column is not supported.
Use `FULL`, `KEYS`, or `VALUES` for frozen map clustering columns.
====
...
include::Cassandra:cassandra:partial$sai/supported-query-operators-list.adoc[]

The unsupported query operators are:

include::Cassandra:cassandra:partial$sai/notSupportedOperators.adoc[]

Examples:

[source,language-cql]
----
include::Cassandra:cassandra:example$CQL/sai/cyclist_semi_pro_select_date_range.cql[tag=cyclistSelectDateRange]
----
...
[TIP]
====
For query examples with `CONTAINS` clauses that take advantage of SAI collection maps, lists, and sets, be sure to see xref:cql/indexing/sai/collections.adoc[SAI collection map examples with keys, values, and entries].
====
...
See xref:Cassandra:cassandra:reference/cql-commands/create-custom-index.adoc[CREATE CUSTOM INDEX] and examples in the xref:Cassandra:cassandra:getting-started/sai-quickstart.adoc[SAI quickstart] topic.
====
...
Refer to xref:cql/indexing/sai/operations/monitoring.adoc#saiIndexGroupMetrics[Index group metrics].
...
See xref:cql/indexing/sai/operations/monitoring.adoc#saiTableStateMetrics[Table state metrics].
...
When evaluating multiple indexed columns in a query, SAI traverses, merges, and intersects the matching index streams across memtables and SSTables.

[IMPORTANT]
====
`AND` queries can use multiple SAI indexes, but if more than two SAI indexes are involved, SAI performs post-filtering on the remaining clauses.
That behavior is important on wide tables and low-cardinality columns because the index may narrow the search less than expected.
====

For related information, see the xref:cql/indexing/sai/sai-concepts.adoc[SAI concepts] warning and the xref:cql/indexing/sai/sai-read-write-paths.adoc#saiMatchStreamingAndPostFiltering[match streaming and post filtering] example.
...
== Can I create an SAI index based on a xref:Cassandra:cassandra:reference/static.adoc[static column]?
...
See xref:Cassandra:cassandra:reference/cql-commands/create-custom-index.adoc[CREATE CUSTOM INDEX].
