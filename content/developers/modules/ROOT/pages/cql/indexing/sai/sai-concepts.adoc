...
Compared with legacy secondary indexes (2i), SAI can combine multiple column indexes in a single query, uses less disk for large indexes, and scales better for mixed read/write workloads.
Use SAI when you need flexible filtering on indexed columns; use 2i only when the query pattern is simple and single-index access is enough.

The main advantages of SAI for {product} are:
...
* text type equality
* `CONTAINS` logic (for collections, including frozen collections in Cassandra 6.0+)
* tokenized data
...
[WARNING]
====
When a query uses multiple indexed predicates joined with `AND`, SAI can combine the indexes, but queries that involve more than two SAI indexes may fall back to post-filtering on the remaining clauses.
Plan those queries carefully, especially on wide tables and low-cardinality columns.
====
...
For building vector search applications with SAI's ANN capabilities, see xref:vector-search/index.adoc[Vector Search].
