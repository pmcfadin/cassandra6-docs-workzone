...
[source,text]
----
Write path: client write -> memtable index -> SSTable flush -> on-disk index files
Read path: query -> choose index -> gather matching partitions -> read rows -> apply final filtering -> return results
----
...
See xref:cql/indexing/sai/operations/monitoring.adoc#saiMonitorMetrics[SAI metrics].
...
See xref:cql/indexing/sai/operations/monitoring.adoc#saiGlobalIndexingMetrics[Global indexing metrics].
...
Unlike legacy secondary indexes, where a query typically uses one index at a time, SAI uses a `Query Plan` that can combine multiple column indexes in one query.

The overall flow of a SAI read operation is:

[source,text]
----
1. Coordinator picks the most selective index.
2. Coordinator sends the read to the relevant token ranges.
3. Replicas intersect matching partitions from the chosen indexes.
4. Replicas apply any remaining filtering while rows are materialized.
5. Matching rows are returned to the client.
----
...
Users can override automatic index selection using xref:cql/dml.adoc#index-hints[index hints] — the `WITH included_indexes` and `WITH excluded_indexes` clauses on `SELECT` statements (CASSANDRA-18112).
