...
The keyspace and table were defined in xref:home:cassandra:getting-started/sai-quickstart.adoc[SAI quickstart].
...
* `TermsLookupLatency` -- For string indexes, such as `country_sai_idx` in the xref:home:cassandra:getting-started/sai-quickstart.adoc[quickstart] examples, this metric shows terms lookup latency percentiles (in microseconds) per one/five/fifteen minute query throughput.
