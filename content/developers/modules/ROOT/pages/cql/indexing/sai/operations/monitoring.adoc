...
For more details, refer to xref:reference::sai-virtual-table-indexes.adoc[Virtual tables for SAI indexes and SSTables].
...
For details about tracing, refer to CQL xref:reference::cqlsh.adoc#tracing[TRACING].
...
The keyspace and table were defined in xref:Cassandra:cassandra:getting-started/sai-quickstart.adoc[SAI quickstart].
...
* `TermsLookupLatency` -- For string indexes, such as `country_sai_idx` in the xref:Cassandra:cassandra:getting-started/sai-quickstart.adoc[quickstart] examples, this metric shows terms lookup latency percentiles (in microseconds) per one/five/fifteen minute query throughput.
