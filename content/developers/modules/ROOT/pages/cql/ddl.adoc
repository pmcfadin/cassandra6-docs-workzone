...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/ks_table.bnf[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/column.bnf[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/options.bnf[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/create_ks.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_ks.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/autoexpand_ks.cql[]
----
will result in:
[source,plaintext]
----
include::Cassandra:cassandra:example$RESULTS/autoexpand_ks.result[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/autoexpand_ks_override.cql[]
----
will result in:
[source,plaintext]
----
include::Cassandra:cassandra:example$RESULTS/autoexpand_ks_override.result[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/autoexpand_exclude_dc.cql[]
----
will result in:
[source,plaintext]
----
include::Cassandra:cassandra:example$RESULTS/autoexpand_exclude_dc.result[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_ks_trans_repl.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/use_ks.bnf[]
----
Using CQL:
[source,sql]
----
include::Cassandra:cassandra:example$CQL/use_ks.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/alter_ks.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/alter_ks.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/drop_ks.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/drop_ks.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/create_table.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_table.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_static_column.cql[]
include::Cassandra:cassandra:example$CQL/insert_static_data.cql[]
include::Cassandra:cassandra:example$CQL/select_static_data.cql[]
----
--
...
[source,sql]
----
include::Cassandra:cassandra:example$RESULTS/select_static_data.result[]
----
--
====
...
|===
|Primary key form |Meaning

|`PRIMARY KEY (a)`
|`a` is the partition key. Every partition contains one row.

|`PRIMARY KEY (a, b, c)`
|`a` is the partition key. `b` and `c` are clustering columns that order rows inside the partition.

|`PRIMARY KEY ((a, b), c)`
|`a` and `b` form the composite partition key. `c` orders rows inside the partition.
|===

----
partition = hash(a, b)
rows within the partition = ordered by c
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_table_single_pk.cql[]
----
...
[IMPORTANT]
====
The primary key uniquely identifies a row in the table.
If another row is inserted using the same primary key, Cassandra performs an upsert and replaces the existing row.
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_table_compound_pk.cql[]
include::Cassandra:cassandra:example$CQL/insert_table_compound_pk.cql[]
include::Cassandra:cassandra:example$CQL/select_table_compound_pk.cql[]
----

will result in
[source,sql]
----
include::Cassandra:cassandra:example$RESULTS/select_table_compound_pk.result[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/create_table_clustercolumn.cql[]
include::Cassandra:cassandra:example$CQL/insert_table_clustercolumn.cql[]
include::Cassandra:cassandra:example$CQL/select_table_clustercolumn.cql[]
----

will result in
[source,sql]
----
include::Cassandra:cassandra:example$RESULTS/select_table_clustercolumn.result[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/select_range.cql[]
----

will result in
[source,sql]
----
include::Cassandra:cassandra:example$RESULTS/select_range.result[]
----
...
| `compaction` |_map_ |_see below_ | xref:operators::operate/compaction-overview.adoc#cql-compaction-options[Compaction options]
| `compression` |_map_ |_see below_ | xref:Cassandra:cassandra:managing/operating/compression.adoc#cql-compression-options[Compression options]
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/alter_table_spec_retry.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/alter_table_spec_retry_percent.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/spec_retry_values.cql[]
----
...
* `'SizeTieredCompactionStrategy'`, xref:Cassandra:cassandra:managing/operating/compaction/stcs.adoc#stcs[STCS] (Default)
* `'LeveledCompactionStrategy'`, xref:Cassandra:cassandra:managing/operating/compaction/lcs.adoc#lcs[LCS]
* `'TimeWindowCompactionStrategy'`, xref:Cassandra:cassandra:managing/operating/compaction/twcs.adoc#twcs[TWCS]

If a custom strategies is required, specify the full class name as a xref:cql/definitions.adoc#constants[string constant].

All default strategies support a number of xref:operators::operate/compaction-overview.adoc#compaction-options[common options], as well as options specific to the strategy chosen. See the section corresponding to your strategy for details: xref:Cassandra:cassandra:managing/operating/compaction/stcs.adoc#stcs_options[STCS], xref:Cassandra:cassandra:managing/operating/compaction/lcs.adoc#lcs_options[LCS], xref:Cassandra:cassandra:managing/operating/compaction/twcs.adoc#twcs_options[TWCS].
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/chunk_length.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/caching_option.cql[]
----
...
_Added in Cassandra 6.0 (CEP-43, CASSANDRA-19964, CASSANDRA-19965, CASSANDRA-20943)._

`CREATE TABLE LIKE` creates a new, empty table by copying the schema of an existing table.

[IMPORTANT]
====
`CREATE TABLE LIKE` copies schema and selected table metadata only. It does not copy data.
Use `WITH INDEXES`, `WITH COMMENTS`, and `WITH SECURITY LABELS` if you want those pieces of metadata copied too.
Custom indexes are not copied.
====
...
like_options ::= like_option [AND like_option]*
...
Grammar source: `src/antlr/Parser.g` `copyTableStatement` and `likeOption` rules; CASSANDRA-19964.

For example, to create a copy of `orders` in the same keyspace:
...
[source,sql]
----
CREATE TABLE reporting.orders_copy LIKE sales.orders;
----

Conditional creation (no error if target already exists):

[source,sql]
----
CREATE TABLE IF NOT EXISTS orders_copy LIKE orders;
----

Copying a table with its SAI and secondary indexes:

[source,sql]
----
CREATE TABLE orders_copy LIKE orders WITH INDEXES;
----

Overriding table options at copy time:
...
* Column definitions: partition key, clustering, regular, and static columns
* Data types, including user-defined types (UDTs)
* Data masking settings
* Table parameters (compaction, compression, and other `WITH` options); these can be overridden at copy time using the `WITH` clause
* SAI and legacy secondary indexes, when `WITH INDEXES` is specified
* Table and column comments, when `WITH COMMENTS` is specified
* Table and column security labels, when `WITH SECURITY LABELS` is specified

=== What is NOT copied

* Data (the new table is always empty)
...
Source and target tables may be in different keyspaces.
When copying across keyspaces, all UDTs referenced by the source table must exist in the target keyspace with matching structure.
Missing or structurally mismatched UDTs cause the statement to fail.

=== Permissions required

* `SELECT` on the source table
* `CREATE` on all tables in the target keyspace

=== Index naming

When `WITH INDEXES` is specified, copied indexes are renamed to avoid conflicts.
System-generated index names are regenerated for the new table.
User-defined index names are prefixed or suffixed to ensure uniqueness within the target keyspace
(source: `src/java/org/apache/cassandra/cql3/statements/schema/CopyTableStatement.java`; CASSANDRA-19965).

=== Guardrails

Guardrails are enforced on the new table as if it were created from scratch.
The following guardrails apply: columns per table, total table count, vector type and dimensions,
compact table creation, uncompressed tables, and table properties.

=== Availability

`CREATE TABLE LIKE` requires cluster metadata serialization version V5 or later.
This feature is available in Cassandra 6.0 and later.

[NOTE]
====
Audit logging records `CREATE TABLE LIKE` operations using the `CREATE_TABLE_LIKE` entry type
in the `DDL` audit category.
====
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/alter_table.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/alter_table_add_column.cql[]
include::Cassandra:cassandra:example$CQL/alter_table_with_comment.cql[]
----
...
It's important to keep in mind that renamed columns shouldn't have dependent secondary indexes.
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/drop_table.bnf[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/truncate_table.bnf[]
----
...
* xref:guides/schema-migrations.adoc[Schema Migrations] -- safe patterns for evolving your schema in production
* xref:guides/time-series-modeling.adoc[Time-Series Modeling] -- partition key and table design for time-series data
* xref:guides/sai-usage-patterns.adoc[SAI Usage Patterns] -- when and how to use Storage-Attached Indexes
* xref:data-modeling/index.adoc[Data Modeling Overview] -- query-driven schema design fundamentals
