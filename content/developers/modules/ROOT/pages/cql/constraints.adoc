...
There is a basic satisfiability check conducted on checks' definitions so we ensure that unsatisfiable constraint
definitions are invalid as it would be impossible to insert a value for a specific column which would satisfy all constraints.
...
----
ALTER TABLE ks.tb ALTER name CHECK LENGTH() < 512;
----
...
(`DESCRIBE` statement on a table will show this fact). This means of constraint definition is just syntactic sugar.
...
Defines a constraint which checks text-like values against a regular expression.
