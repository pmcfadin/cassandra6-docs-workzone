...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/cql_type.bnf[]
----
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/native_type.bnf[]
----
...
[WARNING]
====
Counter columns are not general-purpose integers.
They are not idempotent, they cannot be used in primary keys, they do not support TTL, and retries after a timeout can overcount.
Use them only when those constraints are acceptable.
====

The `counter` type is used to define _counter columns_. A counter column
is a column whose value is a 64-bit signed integer and on which two
...
https://en.wikipedia.org/wiki/Idempotence[idempotent]. An important
...
Values of the `duration` type are encoded as 3 signed integers of
...
Internally, the number of months and days are decoded as 32-bit
integers whereas the number of nanoseconds is decoded as a 64-bit
integer.
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/insert_duration.cql[]
----
...
[WARNING]
====
Collections are intended for small, bounded data. Accessing even one element reads the whole collection, and list updates can be non-idempotent.
If the values can grow without bound, use a table with clustering columns instead.
====
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/collection_type.bnf[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/collection_literal.bnf[]
----
...
Collections are meant for storing or denormalizing relatively small amounts
of data. They work well for things like “the phone numbers of a given
user” or “labels applied to an email”. When items are expected to grow
unbounded, such as “all messages sent by a user” or “events registered by a
sensor”, collections are not appropriate and a specific table with clustering
columns should be used. Concretely, (non-frozen) collections have the
following noteworthy characteristics and limitations:
...
removed or improved upon in the future, it is an anti-pattern to use a
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/map.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/update_map.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/delete_map.cql[]
----
+
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/update_ttl_map.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/set.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/update_set.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/delete_set.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/list.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/update_list.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/update_particular_list_element.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/delete_element_list.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/delete_all_elements_list.cql[]
----

[WARNING]
.Warning
====
Setting and removing an element by position and removing occurrences of
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/udt.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/udt.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/udt_literal.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/insert_udt.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/alter_udt_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/rename_udt_field.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/drop_udt_statement.bnf[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/tuple.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/tuple.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/custom_type.bnf[]
----
