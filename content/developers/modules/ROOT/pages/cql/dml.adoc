...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/select_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/select_statement.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/as.cql[]
----
...
See the xref:cql/functions.adoc#writetime-and-ttl-functions[functions] section for details.
...
a xref:Cassandra:cassandra:developing/cql/indexes.adoc#create-index-statement[secondary index] defined, along with a set value.
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/table_for_where.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/where.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/where_fail.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/token.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/where_group_cluster_columns.cql[]
----

This query will return all rows that sort after the one having “John's Blog” as
`blog_title` and '2012-01-01' for `posted_at` in the clustering order. In
particular, rows having a `posted_at <= '2012-01-01'` will be returned, as
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/where_no_group_cluster_columns.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/where_in_tuple.cql[]
----
...
_Added in Cassandra 6.0 (CASSANDRA-19604, CASSANDRA-19688)._

The `BETWEEN` operator expresses an inclusive range restriction as a single `WHERE` condition:
...
`BETWEEN` is inclusive on both ends: `col BETWEEN x AND y` is equivalent to `col >= x AND col <= y`.
If the lower bound is greater than the upper bound, the query returns an `InvalidRequest` error.

Three forms are supported:

Single-column range on a clustering or non-primary-key column:

[source,sql]
----
SELECT * FROM t WHERE ck BETWEEN 10 AND 20;
----

Multi-column tuple range on clustering columns:

[source,sql]
----
SELECT * FROM t WHERE (ck1, ck2) BETWEEN (1, 'a') AND (3, 'z');
----

Token range on the partition key:

[source,sql]
----
SELECT * FROM t WHERE TOKEN(pk) BETWEEN TOKEN(-9223372036854775808) AND TOKEN(0);
----

`BETWEEN` is also valid in `UPDATE` and `DELETE` `WHERE` clauses for clustering column ranges.

`BETWEEN` on clustering columns does not require `ALLOW FILTERING`.
`BETWEEN` on non-primary-key columns requires either `ALLOW FILTERING` or an
xref:Cassandra:cassandra:developing/cql/indexes.adoc[SAI index] on that column.
When an SAI index is present, the range is pushed down to the index natively
(source: `src/antlr/Parser.g` `relation` rule, `src/java/org/apache/cassandra/cql3/Operator.java`
`BETWEEN(19)`, `src/java/org/apache/cassandra/index/sai/plan/Expression.java`; CASSANDRA-19604, CASSANDRA-19688).

NOTE: `between` is an unreserved keyword in CQL and may be used as a column or table name without quoting.

[[not-operators]]
==== NOT IN, NOT CONTAINS, and NOT CONTAINS KEY operators

_Added in Cassandra 6.0 (CASSANDRA-18584)._

CQL supports negation variants of the `IN`, `CONTAINS`, and `CONTAINS KEY` operators for use in `WHERE` clauses.

`NOT IN` — excludes rows where the column value matches any element of the list:

[source,sql]
----
SELECT * FROM t WHERE status NOT IN ('deleted', 'archived');
SELECT * FROM t WHERE status NOT IN ?;
----

Multi-column tuple form:

[source,sql]
----
SELECT * FROM t WHERE (ck1, ck2) NOT IN ((1, 'a'), (2, 'b'));
----

`NOT CONTAINS` — applies to list, set, and map columns; for maps, matches against values:

[source,sql]
----
SELECT * FROM t WHERE tags NOT CONTAINS 'internal' ALLOW FILTERING;
----

`NOT CONTAINS KEY` — applies to map columns; matches against keys:

[source,sql]
----
SELECT * FROM t WHERE metadata NOT CONTAINS KEY 'secret' ALLOW FILTERING;
----

`NOT IN`, `NOT CONTAINS`, and `NOT CONTAINS KEY` follow SQL three-valued logic (3VL):
comparisons where either operand is `NULL` return `NULL` (unknown) rather than `true` or `false`.
This is consistent with SQL standard behavior and matches the behavior of the positive counterparts
(source: `src/java/org/apache/cassandra/cql3/Operator.java` `NOT_IN(16)`, `NOT_CONTAINS(17)`,
`NOT_CONTAINS_KEY(18)`; `src/antlr/Parser.g` `inOperator` and `containsOperator` rules; CASSANDRA-18584).

`NOT CONTAINS` and `NOT CONTAINS KEY` require `ALLOW FILTERING` or a suitable index.

[NOTE]
====
Cassandra 6.0 also corrects three-valued logic handling for NULL values in comparisons more broadly.
Queries that previously compared nullable columns may see different results after upgrade if they relied
on the prior (incorrect) behavior of non-null results for NULL comparands.
====

[[like-operator]]
==== LIKE operator

_Added in Cassandra 6.0 (CASSANDRA-17198). Previously required a SASI index; now also works with `ALLOW FILTERING`._

The `LIKE` operator matches text column values against a wildcard pattern.
It applies to columns of type `text`, `ascii`, or `varchar`.
`LIKE` requires either an index on the column or `ALLOW FILTERING`.
...
|`'prefix%'` |Starts with |`name LIKE 'Jo%'`
|`'%suffix'` |Ends with |`name LIKE '%son'`
|`'%substring%'` |Contains substring |`name LIKE '%ohn%'`
|`'exact'` |Exact match (no wildcards) |`name LIKE 'John'`
|===

Example using `ALLOW FILTERING`:

[source,sql]
----
SELECT * FROM users WHERE name LIKE 'Jo%' ALLOW FILTERING;
----

When an SAI or SASI index is present on the column, `LIKE` is pushed down to the index rather than
performing a full scan. Without an index, `ALLOW FILTERING` must be specified and a full partition scan
is performed (source: `src/java/org/apache/cassandra/cql3/restrictions/SimpleRestriction.java`,
`src/java/org/apache/cassandra/index/sai/plan/Expression.java` `LIKE_PREFIX`, `LIKE_SUFFIX`,
`LIKE_CONTAINS`, `LIKE_MATCHES`; CASSANDRA-17198).

NOTE: `LIKE` matching is case-sensitive for both `ALLOW FILTERING` and SAI-backed queries unless the SAI
index was created with a case-insensitive analyzer.
...
[WARNING]
====
`ALLOW FILTERING` can force Cassandra to scan every partition it needs to inspect. On large tables this can drive high coordinator load, long latency, and timeouts.
Use it only when the data set is known to be small or when no key design or index can express the query.
====
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/allow_filtering.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/query_allow_filtering.cql[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/query_fail_allow_filtering.cql[]
----

Cassandra cannot guarantee that the scan will stay cheap just because the result set is small.
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/query_nofail_allow_filtering.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/insert_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/insert_statement.cql[]
----
...
See the section on xref:Cassandra:cassandra:developing/cql/json.adoc[JSON support] for more detail.
All updates for an `INSERT` are applied atomically and in isolation.

[WARNING]
====
Unlike in SQL, `INSERT` is an upsert. If the primary key does not exist, Cassandra creates the row; if the same primary key already exists, Cassandra overwrites it.
There is no built-in signal telling you which of those two outcomes happened.
====
...
When using xref:cql/definitions.adoc#prepared-statements[Prepared Statements] bind_markers can be used instead of actual values.

Please refer to the xref:cql/dml.adoc#upsert-parameters[INSERT PARAMETERS] section for information on the `insert_parameter`.
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/update_statement.bnf[]
----
...
See the section on xref:Cassandra:cassandra:developing/cql/counter-column.adoc[counters] for details.
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/update_statement.cql[]
----
...
[source,bnf]
----
include::Cassandra:cassandra:example$BNF/delete_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/delete_statement.cql[]
----
...
xref:cql/dml.adoc#upsert-parameters[updates].
...
[source, bnf]
----
include::Cassandra:cassandra:example$BNF/batch_statement.bnf[]
----
...
[source,sql]
----
include::Cassandra:cassandra:example$CQL/batch_statement.cql[]
----
...
* xref:guides/consistency-choices.adoc[Choosing Consistency Levels] -- decision guide for read/write consistency
* xref:guides/pagination.adoc[Paginating Query Results] -- safe pagination patterns for large result sets
* xref:guides/retries-and-idempotence.adoc[Retries and Idempotence] -- safe retry patterns for DML operations
* xref:guides/adopting-acid-transactions.adoc[Adopting ACID Transactions] -- multi-partition atomic operations
* xref:examples/index.adoc[Code Examples] -- complete CRUD examples in Java, Python, and CQL
