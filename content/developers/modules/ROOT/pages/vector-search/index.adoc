...
[WARNING]
====
Mark vector search as experimental in developer guidance.
Treat ANN search, recall tuning, and production behavior as subject to change while the feature matures.
Validate quality, latency, and operational behavior in your own workload before relying on it for production-critical paths.
====
...
If the model outputs 1536 floats, the schema must use `vector<float, 1536>`.
If the dimensions do not match, writes and ANN queries will fail.
====
...
If you change models later, create a new vector column or a new table and re-embed the stored data.
