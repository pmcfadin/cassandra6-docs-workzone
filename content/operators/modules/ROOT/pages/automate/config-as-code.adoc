...
Before triggering an automated restart, verify whether the changed parameters are live-reloadable.
`nodetool reloadlocalschema` only refreshes schema-related metadata; it is not a general
configuration reload command.
Use the dedicated nodetool command for the setting you changed, or check the parameter's
`restart_required` annotation in the source before rolling the node.
