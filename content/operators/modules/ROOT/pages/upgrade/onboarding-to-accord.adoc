...
that touch both Accord and non-Accord data are split: the Accord
mutations are replayed via the Accord transaction protocol, while
non-Accord mutations follow the standard batch log path. If Accord
replay fails, hint-based fallback is used.
