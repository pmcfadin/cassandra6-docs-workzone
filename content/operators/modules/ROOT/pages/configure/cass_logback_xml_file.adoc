...
It is possible to configure logback.xml in such a way that logs would
appear in `system_views.system_log` table. This is achieved by an
appender implemented in class `VirtualTableAppender`, which is called
`CQLLOG` in the default configuration. When the appender is commented
out, no system logs are written to the virtual table.
...
=== Logging slow queries to Cassandra virtual table
