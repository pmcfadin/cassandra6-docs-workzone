...
// TODO: Further research needed on uncaught_exceptions table:
// - Which code paths call `ExceptionsTable.persist()` (global uncaught-exception handler vs. explicit instrumentation).
// - Whether `exception_location` can be `"unknown"` when the stack trace is empty.
