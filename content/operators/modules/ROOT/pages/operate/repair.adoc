...
Read this page first, then xref:operate/auto_repair.adoc[Auto Repair]
and xref:operate/repair-orchestration.adoc[Repair Orchestration]. This
page explains the manual repair model; the other two pages explain how
to schedule and coordinate it.

[NOTE]
====
If repair does not run before `gc_grace_seconds` expires, deleted data
can reappear from older replicas or from nodes that missed tombstones.
That is the failure mode repair prevents.
====
...
xref:operators::operate/auto_repair.adoc[auto repair] was introduced to
