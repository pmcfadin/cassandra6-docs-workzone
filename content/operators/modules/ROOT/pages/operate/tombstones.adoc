...
Repair has to happen before `gc_grace_seconds` expires if you want to
keep the tombstone safe across replicas. If a replica misses the tombstone
and the cluster purges it too early, the old data can come back during a
later repair or hint replay.
