...
See xref:observe/metrics.adoc#_automated_repair_metrics[Automated Repair Metrics] for the full list of available JMX metrics.

== Considerations

Before enabling Auto Repair, please consult the xref:operate/repair.adoc[Repair] guide to establish a base
...
xref:operate/compaction-overview.adoc#types-of-compaction[anticompactions].
...
xref:reference::sstable/sstablerepairedset.adoc[nodetool sstablerepairedset] to mark all SSTables as repaired
...
In particular one should be mindful of the xref:operate/compaction-overview.adoc[compaction strategy]
you use for your tables and how it might impact incremental repair before running incremental repair for the first
time:

- *Large SSTables*: When using xref:Cassandra:cassandra:managing/operating/compaction/stcs.adoc[SizeTieredCompactionStrategy] or any
  compaction strategy which can create large SSTables including many partitions the amount of
  xref:operate/compaction-overview.adoc#types-of-compaction[anticompaction] that might be required could be
...
- *Partitions overlapping many SSTables*: If partitions overlap between many SSTables, the amount of SSTables included
  ...
  session and must all be anticompacted. xref:Cassandra:cassandra:managing/operating/compaction/lcs.adoc[LeveledCompactionStrategy] is less
  ...
  xref:operate/ucs.adoc#sharding[UnifiedCompactionStrategy's sharding] can also be used to avoid
...
xref:operate/tombstones.adoc#deletion[only_purge_repaired_tombstones] table compaction
...
xref:observe/metrics.adoc#table-metrics[table metrics] can be used to detect inconsistencies in the
repaired data set.
...
is that some tuning might be needed particularly when it comes to tuning how often repair should run
...
avoid xref:operate/tombstones.adoc#zombies[data resurrection].
