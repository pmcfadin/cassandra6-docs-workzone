...
The examples use the keyspaces and tables created in xref:backup-recovery/backups.adoc[Backups].
...
Before you run `sstableloader`, verify the following prerequisites:

* The target keyspace and table already exist.
* The SSTables were produced by a Cassandra version that can be loaded by
  this cluster.
* The directory path matches the target `keyspace/table` layout.
* You know which live nodes to use for `--nodes`.
* The destination cluster has enough free disk and network capacity for the
  stream and the follow-up compaction work.
...
Refer to the sstableloader configuration options at xref:reference::sstable/sstableloader.adoc[sstableloader documentation]
...
Verify the load from the cluster side as well:

* `nodetool status` still shows the target nodes as `UN`.
* `nodetool netstats` or the server logs show the streaming finished cleanly.
* A client query returns the expected row count after the stream completes.
* Pending compactions remain within the range you expect for the imported SSTables.
...
* xref:automate/integrations-overview.adoc[Official Integrations: Sidecar and Analytics] -- for Spark-based bulk read/write workflows that go beyond `sstableloader` and `nodetool import`
* xref:backup-recovery/backups.adoc[Backups and Snapshots] -- snapshot mechanics used by bulk loading
