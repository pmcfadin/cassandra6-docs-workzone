...
consistency like xref:operate/repair.adoc[`anti-entropy repair`] does.
...
xref:contributors::architecture/dynamo.adoc#consistent-hashing-using-a-token-ring[`partitions data across the cluster`] using
...
Hints are a recovery aid, not a durability guarantee. If a node remains
down past `max_hint_window`, repair is what reconciles the missing data
and prevents old values from reappearing after tombstones expire.
...
`max_hint_window` of downtime (defaults to `3 h`). If the
...
coordinator. At this point the client believes the write to be durable
