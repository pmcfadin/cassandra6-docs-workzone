...
Data in {cassandra} is created in xref:contributors::architecture/storage-engine.adoc#memtables[memtables].
Once a memory threshold is reached, to free up memory again, the data is written to an xref:contributors::architecture/storage-engine.adoc#SSTables[SSTable], an https://cassandra.apache.org/_/glossary.html#immutable[immutable] file residing on disk.
...
xref:home:cassandra:managing/operating/compaction/stcs.adoc[`Size Tiered Compaction Strategy (STCS)`]:: 
...
xref:home:cassandra:managing/operating/compaction/lcs.adoc[`Leveled Compaction Strategy (LCS)`]::
...
xref:home:cassandra:managing/operating/compaction/twcs.adoc[`Time Window Compaction Strategy (TWCS)`]::
