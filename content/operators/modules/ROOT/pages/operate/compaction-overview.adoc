...
Data in {cassandra} is created in xref:Cassandra:cassandra:architecture/storage-engine.adoc#memtables[memtables].
Once a memory threshold is reached, to free up memory again, the data is written to an xref:Cassandra:cassandra:architecture/storage-engine.adoc#SSTables[SSTable], an https://cassandra.apache.org/_/glossary.html#immutable[immutable] file residing on disk.
...
For operators, compaction is the mechanism that turns many small or stale
files into fewer, cleaner SSTables. That reduces read amplification and
creates space for tombstones and expired TTL data to be dropped safely.

[source,text]
----
memtables flush to SSTables
        |
        v
compaction picks overlapping SSTables
        |
        v
merge latest rows, keep live data, drop safe tombstones
        |
        v
fewer SSTables and lower disk pressure
----
...
Use the table below as a starting point. It is intentionally conservative:
pick the first strategy that matches the workload, then tune after you
have measured read amplification, write amplification, and tombstone
...
| STCS
| Write-heavy tables or a compatibility fallback
| Large SSTables and more overlap between files

| LCS
| Read-heavy tables with tighter space amplification needs
| Higher write cost and more compaction work in low levels

| TWCS
| Mostly immutable time-series data with TTLs
| Data outside the configured windows is slower to purge

| UCS
| New tables and mixed workloads
| Tuning `scaling_parameters`, `target_sstable_size`, and `base_shard_count`
|===
...
xref:Cassandra:cassandra:managing/operating/compaction/stcs.adoc[`Size Tiered Compaction Strategy (STCS)`]:: 
...
xref:Cassandra:cassandra:managing/operating/compaction/lcs.adoc[`Leveled Compaction Strategy (LCS)`]::
...
xref:Cassandra:cassandra:managing/operating/compaction/twcs.adoc[`Time Window Compaction Strategy (TWCS)`]::
...
Cassandra 6.0 introduces a `compaction_read_disk_access_mode` setting in
`cassandra.yaml` that allows compaction SSTable reads to use Direct I/O
(O_DIRECT), bypassing the OS page cache
(https://issues.apache.org/jira/browse/CASSANDRA-19987[CASSANDRA-19987]).

During compaction, SSTables are read sequentially and the data is rarely
re-read afterward. Without Direct I/O, these reads evict hot data from the
OS page cache, which can increase read latency for concurrent client queries.
Direct I/O prevents compaction reads from polluting the page cache.
...
----
# conf/cassandra.yaml
# Default: auto (inherits disk_access_mode)
compaction_read_disk_access_mode: auto
----

Valid values:

* `auto` (default) -- inherits the `disk_access_mode` setting.
  Preserves pre-6.0 behavior and requires no migration action.
* `direct` -- uses O_DIRECT for SSTable reads during compaction,
...
Only `auto` and `direct` are valid values; other values cause an
...
[NOTE]
====
Benchmarks with a 1 GB hot dataset and 3 GB page cache showed:

* p99 read latency: ~48% improvement (34.6 ms to 18.0 ms)
* Mean read latency: ~25% improvement (1.88 ms to 1.41 ms)
* Page cache stability: ~2.5x more stable during compaction

These improvements are most pronounced under memory pressure, particularly
with Size-Tiered Compaction Strategy (STCS) and large SSTables.
(Source: CASSANDRA-19987 PR benchmarks, Jon Haddad / Sam Lightfoot, 2026-02-04)
====

CAUTION: This setting is opt-in and off by default. Evaluate the performance
impact in your environment before enabling `direct` in production.
`compaction_read_disk_access_mode` defaults to `auto`, which inherits the value of `disk_access_mode` (including `mmap` if that is set).
To use O_DIRECT for compaction reads regardless of the base `disk_access_mode`, set `compaction_read_disk_access_mode: direct` explicitly.
The only accepted values are `auto` and `direct`.
