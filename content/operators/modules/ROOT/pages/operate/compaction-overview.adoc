...
The interaction with `disk_access_mode: mmap` is an open question -- if
`disk_access_mode` is set to `mmap`, `compaction_read_disk_access_mode: auto`
inherits mmap for compaction reads. Set `direct` explicitly to use O_DIRECT
regardless of the base `disk_access_mode`.
