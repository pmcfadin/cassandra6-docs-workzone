...
acheived via xref:developers::cql/security.adoc#grant-permission[`GRANT PERMISSION`].
...
=== Unresolved questions (review required before publication)

[NOTE]
====
*Reviewer note*: The following items require verification against trunk before
this page is considered publication-ready.

. *native_transport_port_ssl silent-ignore vs startup-failure*: Confirm whether
  including `native_transport_port_ssl` in `cassandra.yaml` on 6.0 causes a hard
  startup failure or is silently ignored. `ConfigCompatibilityTest.java` lists it
  in `REMOVED_IN_51` which suggests it may be ignored rather than rejected.
  The upgrade urgency guidance above should be adjusted accordingly.
  (CASSANDRA-19397 research open question 2.)

. *cassandra-env.sh JMX deprecation stance*: The `cassandra-env.sh`
  `configure_jmx` function still works but NEWS.txt describes it as the old way.
  Confirm whether it is formally deprecated in 6.0 or just superseded. Adjust
  the "recommended" framing above if a formal deprecation exists.
  (CASSANDRA-11695 research open question 1.)

. *nodetool JMX port resolution*: When JMX port is configured only in
  `cassandra.yaml` (not in environment), confirm whether `nodetool` resolves the
  port correctly. The JIRA discussion mentions keeping `JMX_PORT` in shell
  scripts for tool compatibility.
  (CASSANDRA-11695 research open question 3.)

. *Crypto providers section*: The 5.0 docs contained a section on
  `DefaultCryptoProvider` and Amazon Corretto Crypto Provider that does not
  appear in trunk docs. Confirm whether this feature was removed from Cassandra
  6.0 or whether the documentation was accidentally dropped.
  (managing-operating delta-catalog coverage gap 1.)
====
