...
* Whether `nodetool cms cancel_in_progress_sequences` can safely cancel operations in any
  phase (PREPARE, START, MID, FINISH) or has restrictions on cancellation phase.
  The source suggests type-specific handling, but the constraints are not fully documented
  from available evidence.
* The `nodetool cms resumedropaccordtable` subcommand is repo-validated
  (source: `src/java/org/apache/cassandra/tools/nodetool/CMSAdmin.java`)
  but its interaction with Accord (CEP-15) is not documented in the available source material.
  This needs clarification before the troubleshooting guide is finalized.
