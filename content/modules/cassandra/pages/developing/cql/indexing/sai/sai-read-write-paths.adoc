...
Users can override automatic index selection using xref:cassandra:developing/cql/dml.adoc#index-hints[index hints] — the `WITH included_indexes` and `WITH excluded_indexes` clauses on `SELECT` statements (CASSANDRA-18112).
