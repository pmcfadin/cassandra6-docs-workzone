...
This page documents the `BEGIN TRANSACTION` CQL statement introduced in Apache Cassandra 6 as part of the Accord distributed transaction system (CEP-15, xref:cassandra:architecture/accord.adoc[Accord]).
It is a user-facing reference for the syntax and semantics of writing multi-partition atomic transactions in CQL.

For the operational guide covering table configuration, migration, and consistency level semantics, see xref:cassandra:managing/operating/onboarding-to-accord.adoc[Onboarding to Accord].
For the internal architecture of the Accord protocol, see xref:cassandra:architecture/accord-architecture.adoc[Accord Architecture].
For the CQL-to-Accord implementation internals, see xref:cassandra:architecture/cql-on-accord.adoc[Developers guide to CQL on Accord].
...
See xref:cassandra:managing/operating/onboarding-to-accord.adoc[Onboarding to Accord] for enablement steps.
...
Accord transactions provide **serializable isolation**.
...
Source: xref:cassandra:architecture/cql-on-accord.adoc[CQL on Accord], "Supported consistency levels" section.

The effective consistency level also depends on the table's `transactional_mode` and its migration state.
See xref:cassandra:managing/operating/onboarding-to-accord.adoc[Onboarding to Accord] for the full consistency semantics table.
...
`BEGIN TRANSACTION` statements cannot be used as traditional CQL prepared statements (the `PREPARE` command will succeed but the statement is not cached in the usual way).
Source: `TransactionStatement.java` `eligibleAsPreparedStatement()` returns `false`.

[NOTE]
====
Driver-level prepared statement APIs (Java driver `session.prepare(...)`) do work for transactions in practice, as shown in
https://github.com/apache/cassandra/blob/trunk/test/unit/org/apache/cassandra/cql3/PreparedStatementsTest.java[`PreparedStatementsTest.java`] lines 796-810.
The server-side behavior of `eligibleAsPreparedStatement()` returning `false` means the statement is not eligible for the server-side prepared statement cache.
The exact driver interaction semantics should be confirmed before publication.
====
...
* xref:cassandra:architecture/accord.adoc[Accord] — Overview of the Accord transaction protocol.
* xref:cassandra:architecture/accord-architecture.adoc[Accord Architecture] — Internal implementation details for contributors.
* xref:cassandra:architecture/cql-on-accord.adoc[Developers guide to CQL on Accord] — How CQL statements map to Accord transactions internally.
* xref:cassandra:managing/operating/onboarding-to-accord.adoc[Onboarding to Accord] — Operator guide: enabling Accord, configuring tables, migration procedures.
* xref:cassandra:developing/cql/dml.adoc[Data Manipulation Language] — Standard INSERT, UPDATE, DELETE, SELECT reference.

== Unresolved Questions

The following items require verification or additional research before this page can be promoted from draft status:

. *Prepared statement behavior*: The interaction between `eligibleAsPreparedStatement() = false` and driver-level `PREPARE` needs clarification. The server-side vs. driver-side semantics are not fully documented in trunk sources.
