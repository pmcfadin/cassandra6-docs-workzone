...
to check for and refresh compression dictionaries cluster-wide.
Accepts duration units (e.g., `3600s`, `1h`).
Newly trained dictionaries will be picked up by all nodes within this interval.
Source: `src/java/org/apache/cassandra/config/Config.java` line 522.
* `compression_dictionary_refresh_initial_delay` (default: `10s`): Initial
delay before the first dictionary refresh check after node startup.
Accepts duration units (e.g., `10s`).
Source: `src/java/org/apache/cassandra/config/Config.java` line 523.
...
cache entry TTL. Expired entries are evicted and reloaded on
next access. Accepts duration units (e.g., `24h`, `86400s`).
Source: `src/java/org/apache/cassandra/config/Config.java` line 525.
