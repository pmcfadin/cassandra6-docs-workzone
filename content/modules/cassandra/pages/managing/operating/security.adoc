...
`remote` (boolean)::
  ...
  `jmx_encryption_options` (see xref:cassandra:managing/operating/security.adoc#jmx-with-ssl[JMX With SSL]).
...
`jmx_encryption_options` (nested block)::
  ...
  xref:cassandra:managing/operating/security.adoc#jmx-with-ssl[JMX With SSL] for
...
. If found, remove the property and configure `client_encryption_options` as
  described in xref:cassandra:managing/operating/security.adoc#native-transport-port-ssl-removed[the removal notice above].
...
See xref:cassandra:managing/operating/security.adoc#jmx-yaml-migration[Migrating JMX configuration from cassandra-env.sh to cassandra.yaml] for step-by-step instructions.
