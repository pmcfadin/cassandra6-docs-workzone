...
1. The canonical YAML key for the address configuration setting should be confirmed against trunk `conf/cassandra.yaml` line 1649 — the research notes a discrepancy between `addresses_config` (YAML) and `address_config` (NEWS.txt singular form).
2. Whether `RackInferringSnitch` has a direct `InitialLocationProvider` equivalent should be verified against trunk source.
3. Whether `GoogleCloudSnitch` and `CloudstackSnitch` are fully removed from trunk source or only deprecated should be confirmed before marking them as removed.
4. Timeline for removal of `IEndpointSnitch` support is not yet documented; this affects how urgently migration guidance should be presented to operators.
====
