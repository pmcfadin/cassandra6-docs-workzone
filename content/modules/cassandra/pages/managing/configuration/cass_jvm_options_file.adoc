...
[NOTE]
====
The following items require verification before this page is promoted from draft status:

* Confirm whether CASSANDRA-18831 is the sole JIRA for the ZGC-as-default change or whether a sub-task covers that decision separately.
* Confirm recommended heap size ranges for ZGC vs G1GC on typical Cassandra workloads (not documented in `jvm21-server.options` as of trunk research date).
* Verify whether `cassandra-env.sh` JDK detection logic requires documentation updates.
* Confirm interaction with CASSANDRA-20980 (concurrent GC thresholds) — consider co-locating or cross-referencing that content here.
* Technical review requested from Josh McKenzie or Mick Semb Wever (CASSANDRA-18831 patch authors/reviewers).
====
